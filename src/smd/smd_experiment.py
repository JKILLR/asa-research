#!/usr/bin/env python3
"""
SMD (Semantic Molecular Dynamics) Experiment.

Tests energy minimization attention vs standard attention.
The article's Level 3 vision: bonding IS computation.

Configs:
1. Standard 8 heads (baseline)
2. Combined 1proj+7std+gate (our best, validated)
3. SMD 2smd+6std+gate (2 energy minimization heads + 6 standard)
4. SMD 4smd+4std+gate (half energy minimization)

WikiText-2, d=256, 3 seeds, 10 epochs.
"""
...
def main():
    ...
    configs = {
        'standard': {
            'desc': 'Standard 8 heads',
            'fn': lambda: StandardLanguageModel(vocab_size, d, L, 8, dk, ff, seq_len),
            'ds': (train_ds_zero, val_ds_zero),
        },
        'combined': {
            'desc': 'Combined 1proj+7std+gate',
            'fn': lambda: CombinedLanguageModel(vocab_size, d, L, 1, 7, feature_dim, dk, ff, seq_len, use_wave_gate=True),
            'ds': (train_ds, val_ds),
        },
        'smd_2_6': {
            'desc': 'SMD 2smd+6std+gate (4 steps)',
            'fn': lambda: SMDLM(vocab_size, d, L, 2, 6, feature_dim, dk, ff, seq_len, n_steps=4, use_wave_gate=True),
            'ds': (train_ds, val_ds),
        },
        'smd_4_4': {
            'desc': 'SMD 4smd+4std+gate (4 steps)',
            'fn': lambda: SMDLM(vocab_size, d, L, 4, 4, feature_dim, dk, ff, seq_len, n_steps=4, use_wave_gate=True),
            'ds': (train_ds, val_ds),
        },
    }
    ...
    for cfg_name, cfg in configs.items():
        ...
        with open('/home/asa/asa/smd_results.json', 'w') as f:
            json.dump(results, f, indent=2)

    std = results['standard']['losses']
    print(f"\n{'='*60}", flush=True)
    print("SMD EXPERIMENT — RESULTS", flush=True)
    print(f"{'='*60}", flush=True)
    for name, r in results.items():
        t, p = stats.ttest_ind(r['losses'], std)
        gap = (r['mean_ppl'] - results['standard']['mean_ppl']) / results['standard']['mean_ppl'] * 100
        sig = ' ★' if p < 0.05 else ''
        print(f"  {r['desc']:<35s} PPL={r['mean_ppl']:.1f}±{r['std_ppl']:.1f} {gap:+.1f}% p={p:.4f}{sig}", flush=True)
